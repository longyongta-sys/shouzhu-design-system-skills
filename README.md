...
这是首住后台管理系统的 AI agent 设计规范 skill 交付包，适用于 Vue 3 + Element Plus 项目。

这套 skill 的目标是让产品、UI、开发在生成原型或页面时，优先遵循首住后台的布局、菜单、组件样式、交互状态和业务展示规则，从而减少 AI 自由发挥导致的样式偏差。
...
这套规范不是替代 Element Plus，而是在 Element Plus 的基础上做首住后台样式约束。
